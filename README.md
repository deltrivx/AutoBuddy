...
- **只认「上游明确说不可用」才写禁用**：`429` 限流、超时与网络异常一律**跳过、不改配置** ——
  上游高峰期限流是常态，把它当故障会在高峰后留下一批被误禁的好模型；
  探测请求自身被上游参数校验拒绝时同样跳过（那说明探测有问题，对模型一无所知）。
- **账号凭据失效不牵连模型**：某个账号的鉴权整体失效（token 过期最常见）时，
  本轮**整体跳过该账号**并在面板上点名提示重新登录 —— 坏的是凭据不是模型，
  逐条禁掉它名下所有模型解决不了问题，只会在重新登录后留下一批需要手动放开的误禁。
...
- **同一份策略**：巡检写入的就是手动禁用用的那一份配置，**关掉巡检后已写入的禁用项继续生效**，
  随时可以在账号卡片上手动启用 / 重新禁用。
- **整轮误判保护**：一整轮里若没有任何组合可用、且失败数量达到门槛，
...
项目主页 / 更新日志 / 发布版本 / 问题反馈入口、运行位置（数据目录 / 控制台 / 网关 API，
数据目录可一键复制）、当前规模（账号 / 模型 / 禁用组合，按来源拆分），以及非官方项目声明。

这里显示的就是**唯一的版本号** —— 与 GitHub 发布标签、镜像 tag 同号，
不用再去别处对一遍。构建流水线会校验两者一致，对不上直接失败。

项目地址与项目名由服务端下发，可用环境变量覆盖 —— 二次分发的版本不会把使用者引回本仓库。
...
- **网关转发的调用**：输入 / 输出 / 缓存命中数全部取自上游随响应返回的**实测用量**，
  「缓存命中率」即由这套数据算出，与官方客户端的口径一致。
- **调用方中途断开**时收不到上游的末帧，这类记录回退为按文本长度估算，缓存命中记为 0
  （不知道就是不知道，不猜一个数出来）。
- **从官方云端流水合并进来的记录**只有「积分」，其 Token 是按固定比例**折算的估算值**，
  且没有缓存明细。两类记录会出现在同一张报表里，判断绝对量级时请注意这一差异。
