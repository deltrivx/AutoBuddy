...
### 🚦 模型级禁用：点一下就拉黑「某账号的某个模型」

**为什么做这个**：账号池管的是「哪个账号参与调用」，但实际用起来还有更细的一层 ——
一个账号有几十个模型，其中**个别模型**因为优惠政策调整或免费额度到期，在这个账号上已经调不通了，
可这个账号的**其他模型仍然好用**。停用整个账号是过度反应，放任不管又会让轮询时不时撞上死模型。

所以禁用粒度落在「**这个账号的这一个模型**」。

**新增：模型标签可点击（账号卡片）**

- 账号卡片下方每个模型标签都变成可点击的，**点一下禁用、再点恢复**，无需进设置页。
- 标签扩展为三态：默认（可用）/ 蓝底加粗（该账号真实调用过）/ **灰底删除线（已禁用）**。
- **禁用优先于已用**：禁用态覆盖蓝色高亮，否则刚点掉的那个看起来「没反应」。
- 卡片标题旁新增 `已禁用 N` 徽章，账号卡片折叠时也能看出这里动过手。
- 支持键盘（`Tab` 聚焦 + `Enter` / `Space`）。悬停才显边框，一排标签不会看着像按钮墙。

**新增：网关侧「模型感知」的账号选择（`gateway/main.py`）**

- `select_account()` 新增 `model` 参数：把禁用了该模型的账号从候选里剔除，只影响**本次请求**。
- 模型在**选账号之前**就取出，因此路由能提前避开死组合，而不是等上游报错。
- **别名归一**：禁用 `hy4` 会连同目标模型 `hy3` 一起挡住 —— 否则调用方换个写法就绕过了。
- **安全回退**：某模型在所有账号上都被禁用时，回退到不过滤而非返回「无账号可用」。
  在网关层编一个错误会掩盖上游的真实原因，不利于排查。
- 不传 `model` 时行为与升级前**完全一致**（不过滤）。

**新增：模型策略接口与持久化**

- `GET /account-models/config` 读取策略；`PUT /account-models/config` 切换单点或整份覆盖。
- 落盘 `/data/.wb-switch/model_policy.json`，与 `account_pool_config.json` **完全分离、互不覆盖**。
- 只记**禁用项**，未列出的一律可用 —— 官方上新模型时天然可用，不需要迁移配置。
- `tmp` + `os.replace` 原子替换；文件缺失 / 损坏 / 结构不对一律回退空策略，绝不抛异常。
- `/api/account-models` 每账号新增 `id` 与 `disabled` 字段供 UI 渲染；`/account-pool/status`
  新增 `modelPolicy`；`/gateway/info` 新增 `models.disabledCombos`。
- 兜底：某账号「一条调用记录都没有但已被禁用过模型」时也会被补进结果，
  否则它的禁用状态会静默丢失。

**新增：WebUI 代理转发**

- `PUT /api/account-models` 转发禁用切换，把网关的 `detail` 原样带回，错误能弹出来。

**验收**

- 新增 `_test_model_policy.py`（**13 项断言**）：别名归一、读写回环、损坏 / 非 dict 回退、
  原子写无 `.tmp` 残留、去重排序。
- 新增 `_test_model_routing.py`（**8 项断言**）：无策略时轮询覆盖全账号、禁用后不再落到该账号、
  其他模型不受影响、别名禁用同样生效、全禁用时回退而非 `None`、不传 `model` 不过滤、
  显式指定被禁用的组合返回 `None`。
- 两个测试均 stub 掉 fastapi / httpx，**无需安装依赖**。
- 不改变账号管理、自动签到、账号池调度与 Token 统计的既有行为。
...
**为什么做这个**：容器只有两个核心能力 —— 账号管理/自动签到，以及对外提供 OpenAI 兼容 API。
但第二个能力在 UI 上此前**完全不可见**：接入地址要翻 README，也没有任何密钥相关的配置项。
本次把「怎么连、用什么密钥连」直接落到 **设置 → API 接入**。

**新增：设置页「API 接入」面板**

- **对外访问地址**：按当前访问的主机名自动推导为 `http(s)://<host>:18091/v1`，可手动覆盖（走域名/隧道时用），改动存在浏览器本地并可一键恢复默认。
- **接口端点**：`POST /v1/chat/completions`、`GET /v1/models`，点一下复制完整地址。
- **网关状态**：版本、账号池模式、可路由模型数、可用账号数与参与调用的账号数，以徽章呈现。
- **密钥校验开关**：「强制 API 密钥校验」一键开/关，**默认关闭**，关闭时行为与升级前完全一致。
- **密钥管理**：新建 / 复制明文 / 停用 / 启用 / 删除 / 清空；每行显示掩码、调用次数、最近使用时间，悬停可见创建时间。
- **连通性自检**：容器内回环实测「网关健康 / 模型清单 / 密钥配置自洽性」三步，用来定位「UI 能打开但 API 调不通」。
- **调用示例**：自动带上当前启用密钥的 `curl` 片段，另有「复制接入信息」一键导出 Base URL + 端点 + 鉴权 + 密钥 + 模型数。

**新增：API 密钥（`gateway/api_keys.py`）**

- 格式 `sk-wb-` + 32 位十六进制；校验接受 `Authorization: Bearer <key>` 与 `x-api-key: <key>`，`Bearer` 大小写不敏感，首尾空格容忍。
- 缺失 / 错误 / 已停用的密钥一律 `401`，错误信息为可读中文。
- 以 `secrets.compare_digest` 做常量时间比较，避免时序侧信道。
- 落盘 `/data/.wb-switch/api_keys.json`（权限 `0600`），整份重写 + `tmp`/`os.replace` 原子替换；文件损坏或结构不对一律回退到「未启用校验 + 空列表」，**绝不抛异常把网关打死**。
- 调用计数与最近使用时间在内存累加、磁盘最多每 5 秒写一次；统计失败一律吞掉，绝不影响 API 请求。

**安全设计：两道门锁防线**

1. 一个可用密钥都没有时，**不允许开启校验**；
2. 已开启校验时，**不允许停用/删除最后一个启用中的密钥**，也**不允许清空全部**。

这两条挡的都是同一类事故：一键把自己和所有下游客户端关在门外，之后只能进容器手改文件才能救回来。
清空全密钥时不会「顺手帮你把校验关掉」—— 静默降低安全性比多一次点击更糟。

**安全设计：容器内回环免校验**

WebUI 代理（18090）与网关（18091）同容器，它需要读 `/v1/models` 来渲染账号卡片的模型清单；
若一并拦截，开了密钥校验就会把 UI 自己弄坏。容器外的请求经 docker NAT 进来源地址是网桥地址而非
`127.0.0.1`，因此放行回环不会把外部请求放进来。

**顺带修掉一个真实隐患：内部调用被宿主代理劫持**

`httpx` 默认 `trust_env=True`，会读取宿主机上的 `HTTP_PROXY` / `HTTPS_PROXY` / `ALL_PROXY`。
NAS 上为了下载或科学上网设全局代理很常见，一旦被代理接管，容器内的 loopback 调用会被转发到代理并
返回 `404` —— 表现为「UI 打不开 / 模型清单为空」，但网关本身是好的。现在 WebUI 代理的全部内部调用
统一走 `trust_env=False`；**网关访问上游模型服务的客户端保持默认**，那里恰恰需要代理环境变量才能出网。

**验收**

- 新增本地单测 `_test_api_access.py`（stub 掉 fastapi / httpx，**无需安装依赖**），**80 项断言全部通过**，覆盖：密钥格式与掩码、增删改停用、常量时间比对、`_gateway_auth` 的 loopback 放行规则（`127.0.0.1` / `::1` / 外部地址 / `x-api-key` / `Bearer` 大小写）、两道门锁防线、文件损坏回退、无 `.tmp` 残留、连接信息接口自洽。
- 另做 HTTP 级集成验证：真起 18090 + 18091 两个服务，覆盖密钥全生命周期、走代理转发的增删改、门锁防线、路由优先级（专属路由未被 catch-all 抢走）、自检三步。
...
### 💾 选账号流水落盘：账号调用次数不再随容器重建归零

**现象**：账号卡片上的「已调用 N 次」在容器重建后会归零，需要查明原因。

**诊断结果（确认是，且只有它是）**：

| 计数 | 数据源 | 存储 | 容器重建后 |
| :--- | :--- | :--- | :--- |
| 账号池条 `已调用 N 次` | `main.py` 的 `_SELECTION_LOG` | **进程内存** | **清零** |
| `gatewayCalls`（v0.3.16 起前端不显示） | `token_stats_logs.json` | **磁盘** | 保留 |

铁证：新容器启动于 `17:51:17`（UTC），`[pool]` 日志最早一条 `17:51:19`，之前历史全部消失；
而 `token_stats_logs.json` 里 285 条记录最早可追溯到 `09-18 03:11`（跨重建保留）。

**修复**：把 `_SELECTION_LOG` 落盘到 `/data/.wb-switch/selection_logs.json`。

- `_load_selection_log()` —— 模块导入时恢复；文件缺失/损坏/结构不对一律返回空列表，**绝不抛**；
  逐条校验 `accountId` 存在，并裁到环形上限 200。
- `_save_selection_log_locked()` —— **整份重写**（上限 200 条约 20 KB，比追加更不容易写出半截 JSON），
  `tmp + os.replace` 原子替换，进程被 kill 也不留坏文件；异常一律吞掉 —— **统计落盘失败绝不影响 API 请求**。
- 写盘放在 `_SELECTION_LOCK` **内**：保证「后发生的 append」一定写出更新的整份快照，不会乱序覆盖。
- `POST /account-pool/selections/reset` 现在**同时清内存与磁盘**，否则下次重启会把旧流水读回来，
  看起来像「清零没生效」。

**⚠️ 语义澄清（重要）**：落盘只是让计数**跨重启连续**，**不是**变成历史累计值。
它依然是**滑动窗口** —— 统计「最近 200 次选账号里各账号出现几次」；
`gatewayCalls` 同理，窗口是最近 3000 次调用。两者窗口大小不同，都随新请求滑动而衰减。

**验收**：新增本地单测 `_test_selection_persist.py`（stub 掉 fastapi/httpx，无需装依赖），
22 项断言全通过 —— 覆盖首次启动无文件、记录即落盘、重载恢复、环形上限裁剪、
reset 双清、重启后仍为空、文件损坏容忍、结构非法容忍、条目过滤、写盘失败不影响请求路径、无 tmp 残留。
...
- **v0.3.4 只移除了「底部 footer」那一份，漏掉了卡片头部账号名右侧的图标版**。官方 UI 里这组产品槽位存在两份：
  ...
  2. **头部图标行** `div.ml-auto.flex.shrink-0.items-center.gap-1`（紧凑图标）—— **本次移除**。这才是「账号后面一个 CodeBuddy IDE 和 CodeBuddy CLI 按钮」的实际位置。
