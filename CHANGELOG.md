...
### 🔎 二轮验收：账号页副标题仍在宣称「管理 CodeBuddy IDE 与 CodeBuddy CLI 账号」
...
这句话在账号页顶部**常驻渲染**，却把 `CodeBuddy IDE` / `CodeBuddy CLI` 说成本页的管理对象——
而这两个桌面端产品早在 v0.3.11 就被物理移除，容器里根本不存在。用户看到的第一句话就在描述不存在的功能。
