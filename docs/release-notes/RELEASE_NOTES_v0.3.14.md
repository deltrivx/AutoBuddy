# v0.3.14 - 二轮验收：账号页副标题在二进制里仍宣称管理 CodeBuddy IDE 与 CLI

> ⚠️ **更正**：本版初版把下面这条写成「用户可见的真问题」，**是错的**。见 v0.3.15：
> 响应层（`web_proxy.py` 的 `clean_mac_content`）自 v0.3.8 起就在改这句文案，
> 用户看到的一直是改写后的版本。真实性质是**源码级残留**，不是用户可见缺陷。

> 承接 v0.3.13。这轮换了个更严的筛法，又找出 **1 处源码级残留**（已修），并**修正了上一版对 `AZ` 弹窗的错误判定依据**。
...
它把 `CodeBuddy IDE` / `CodeBuddy CLI` 说成本页的管理对象——而这两个桌面端产品早在 v0.3.11 就被物理移除，容器里根本不存在。（浏览器里这句会被响应层改写掉，见 v0.3.15；但二进制源头确实留着这段陈述不存在功能的文案。）
