...
- **积分统计页新增「总积分趋势」折线图**：官方页面原本只有「消耗（柱状）」与
  「按模型分类（横向条形）」，现按同一套卡片样式补齐总积分曲线，插在消耗趋势之前。
  数据两条腿走路：**倒推**（用官方 `daily[]` 每日消耗从当前剩余往回累加，
  一上线即有完整曲线）+ **实测快照**（每次成功取数把当天剩余额度落一条，
  逐渐覆盖倒推值，图上实心点为实测、空心点为推算）。

### 变更

- **签到状态接口 60 秒缓存**：`/api/checkin/status` 上游要 1.46s、经反代 2.34s
  （需逐个账号实时查询 WorkBuddy 服务端），而 `travel/status` 等走本地 SQLite
  仅 0.03s。命中缓存直接返回，刷新页面不再等两秒。只缓存成功响应，
  失败不缓存，避免一次上游抖动被放大成整分钟的「查不到签到状态」。

### 修复

- **修复签到徽章显示明显滞后于飞机旅行图标**：原先前端用 `Promise.all` 等三个接口
  齐了才渲染，签到状态是最慢的那个，于是飞机图标秒出、签到徽章要等两秒多。
  现改为**各自到齐就渲染**：账号列表与签到日志（0.03s）先出，签到状态到达后
  局部补写；状态未到时徽章显示「查询中…」而非误报「暂无记录」。
- **Buddy 徽章移除外围虚线框**（承接 v0.9.1）：仅保留图标本身的虚线绘制与配色，
  三态外形视觉统一。
...
[未发布]: https://github.com/deltrivx/AutoBuddy/compare/v0.9.2...HEAD
[v0.9.2]: https://github.com/deltrivx/AutoBuddy/compare/v0.9.1...v0.9.2
