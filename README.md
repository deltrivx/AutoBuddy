...
- **检测账号**：卡片上一键发轻量鉴权请求，回报凭据是否有效，不改配置、不耗额度。
  ...
  出错也只补一句建议（如「账号被上游拦截，暂时用不了」），
  判定依据与完整成因留在悬浮说明与接口返回里，需要排查时可见，日常不用读。
  **配色即结论**：绿色=正常、琥珀色=被拦截、红色=登录过期。
  提示条限宽并自动换行，窄屏不溢出。
...
- **账号级停用与恢复**：确认凭据失效、或被上游拦截的账号会自动停用（可关）；
  拿不到结论时不停用。账号级探测判定恢复正常后，自动放回轮询；
  账号被拦截期间冻结其模型级自动启用。人工停用的账号巡检不会替人放开。
...
| 账号被上游拦截 | 登录是好的，但请求在模型校验前就被拦下 | 重新扫码无效，需要换一个账号；巡检会自动停用 |
