...
分类只允许使用下面六种，不得自造分类名
（曾出现过「优化」「设计边界」等自造标题，会让版本索引的摘要列失准）：
...
每个版本标题下方**必须**写一行摘要注释，供 `scripts/gen_releases.py` 生成版本索引：

```markdown
## [vX.Y.Z] - YYYY-MM-DD

<!-- summary: 一句话说清这个版本做了什么 -->

### 新增
```

部署与升级请看 [README](./README.md)。

---

## [v0.9.5] - 2026-09-26

<!-- summary: 取消昵称统一显示名、头像预览独立放大、CHANGELOG 与 README 规范化 -->

### 变更

- **取消昵称，显示名以用户名为准**：此前「用户名」与「昵称」是两套并存的显示名，
  同一身份存在两种答案。现移除昵称字段（设置页输入框、修改资料弹窗、用户球浮窗的
  `@用户名` 重复行一并清理），`/api/auth/profile` 与 `/api/auth/status` 不再返回
  `nickname`；显示名统一取登录用户名。`db.get_nickname` / `set_nickname`
  保留但标记为遗留，不再被任何代码调用。
- **头像预览独立成行并放大**：预览图由 52px 提升至 88px，从「与选择器并排」
  改为「居中独占一行」，选择器整行排在下方。此前预览被挤在左侧，难以判断
  所选头像的实际效果。
- **CHANGELOG 规范化**：分类只允许 `新增 / 变更 / 废弃 / 移除 / 修复 / 安全` 六类
  （此前出现自造的「优化」「设计边界」等标题，导致版本索引摘要列失准）；
  并为缺失的版本补回 `<!-- summary: … -->` 摘要注释 —— 该机制自 v0.5.11 起中断，
  造成 `RELEASES.md` 摘要列长期只显示「新增」「修复」这类分类名。
- **README 重写为规范化文档**：移除营销式表述与 emoji 标题，改为
  功能 / 快速开始 / 配置 / 接入 / 运维 / 实现说明 / 许可 / 致谢 的克制结构，
  新增「实现说明」章（目录结构、界面注入方式、构建约束），便于二次开发。

## [v0.9.4] - 2026-09-26

<!-- summary: README 重写为可照做的文档，后台日志彻底降噪，头像上传钮排版修正 -->
...
<!-- summary: 移除「账号接入」（GitHub 自动注册）功能，修复误删的每日任务与 Buddy 徽章函数 -->
### 移除
...
<!-- summary: 修复能力矩阵重入死锁导致网关 502 / 挂起，统一 Buddy 徽章图标 -->
### 修复
...
<!-- summary: 模型失败即时学习并换号重试、正向能力矩阵、后台日志去重、Buddy 徽章统一图标 -->
### 新增
...
<!-- summary: 账号并发分摊、头像全面圆形化、用户球悬浮展开、密码位数掩码 -->
### 新增

- **账号并发分摊**：选账号从「固定顺序单指针轮询」改为「在飞请求数最少优先」——并发请求会自动落到当前最空闲的账号，不再出现慢请求反复压在同一个账号上的情况；状态接口新增 `inflight` 字段，可实时查看各账号在飞的请求数。

### 变更

- **头像全面圆形化**：官方图标改由后端 SVG 出口提供「浅蓝纯圆底衬 + 居中 logo」，彻底解决方形图标硬裁切导致的边缘不规则；所有头像（官方 / 预设 / 自定义）统一正圆，右侧留白一致。
- **右下角用户球默认纯圆形**：平时是 44px 正圆头像（无文字、无内边距错位），点击或鼠标悬浮都会展开成胶囊并显示昵称与浮窗，点击页面其他位置或移开鼠标自动收纳。
- **设置页密码展示**：移除「会话有效期 N 小时，到期后需重新登录」提示；已设定密码时按实际位数用等宽 `*` 掩码展示（不泄露任何字符）。
- **头像选择器尺寸统一**：官方与预设图标统一 42px 圆形选中框，选中态蓝色描边，消除此前官方项空白/大小不一的问题。

---

## [v0.8.1] - 2026-09-26

<!-- summary: 品牌统一、浮窗积分信息、无 Buddy 虚线徽章、头像体系升级 -->
### 修复
...
<!-- summary: 右下角用户球与账号资料管理，会话收紧为 24 小时 -->
### 新增
...
[未发布]: https://github.com/deltrivx/AutoBuddy/compare/v0.9.5...HEAD
[v0.9.5]: https://github.com/deltrivx/AutoBuddy/compare/v0.9.4...v0.9.5
