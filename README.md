...
- **手动首选**：可固定只用某一个账号，再点一次回到自动分配。
  「设为首选」只改用哪个账号，**不会动你勾选的参与调用名单**。
- **按账号停用**：一键让某个账号退出自动轮询，再点一次恢复；
  显式指定它的请求仍可用（停用针对轮询，不是禁止调用）。
- **检测账号**：卡片上一键发轻量鉴权请求，回报凭据是否有效，不改配置、不耗额度。
  结果以提示条呈现，**一句话说清能不能用**：正常即「账号正常，可以放心使用」；
  出错也只补一句建议（如「账号被上游风控拦截，暂时用不了 重新登录没用」），
  判定依据与完整成因留在悬浮说明与接口返回里，需要排查时可见，日常不用读。
  **配色即结论**：绿色=正常、琥珀色=被风控、红色=登录过期。
  提示条限宽并自动换行，窄屏不溢出。
...
- **一键恢复**：卡片标题行的「全部恢复 N」把该账号被禁用的模型一次放回，
  含手动禁用与巡检禁用。原先只能逐个点标签，巡检误禁一批时点起来很碎。
...
- **账号级停用与恢复**：确认凭据失效、或被上游风控的账号会自动停用（可关）；
  拿不到结论时不停用。账号级探测判定恢复正常后，自动放回轮询；
  账号被风控期间冻结其模型级自动启用。人工停用的账号巡检不会替人放开。
- **整轮误判保护**：整轮都没有可用信号时判定为探测机制本身出了问题，整轮作废、不写配置。
- **不拖慢网关**：探测在独立线程执行，且同一时刻只允许一轮。

**判定标准**：一个账号的状态拆成两个独立维度，各自裁定、互不反推。

| 维度 | 怎么测 | 怎么判 |
| :--- | :--- | :--- |
| 凭据有效性 | 用一个**不存在**的模型名请求 | 上游说「模型不存在」= 认下凭据；鉴权类拒绝 = 凭据失效 |
| 模型可用性 | 用**真实**模型名请求 | 只看上游错误码，内容审查 / 参数错误不算模型不可用 |

**凭据有效性只由账号探测裁定** —— 不从模型探测结果反推。同一个 403 可能是凭据过期、
内容审查、无模型权限，只看状态码必然误判；只在「确凿的模型不可用」时才写禁用策略。

### 🔎 一致性自检

凭据与模型是两个维度，只看一个容易得出片面结论。自检把两侧结论并排列出：

- **逐账号对照**：每个账号给出凭据结论、模型结论、判定依据与建议。
- **只读**：不改任何配置，随时可跑。
- **标准可查**：判定标准随结果一并给出，不必翻文档。

| 结论 | 含义 | 建议 |
| :--- | :--- | :--- |
| 账号正常 | 凭据可用，模型请求也放行 | 无需处理 |
| 账号被风控 | 凭据有效，但请求被上游拦截 | 重新登录无效，需确认账号状态；巡检会自动停用 |
| 登录已过期 | 上游以鉴权类错误拒绝 | 重新登录 |
| 没测出结果 | 探测被拒、网络异常或限流 | 稍后重试 |
...
| `GET /account-health/audit` | 一致性自检：逐账号对照凭据与模型两侧结论 |
